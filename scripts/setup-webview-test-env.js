...
async function main() {
	...
	if (!existsSync("./test-webview")) {
		try {
			...
			await execShellCommand(
				"npx degit microsoft/vscode-webview-ui-toolkit-samples/default/component-gallery test-webview",
			);
		} catch (err) {
			console.log(
	...
	try {
		...
		createDir(
			"./test-webview/node_modules/@vscode/webview-ui-toolkit/dist",
		);
		copyDir(
			"./dist",
			"./test-webview/node_modules/@vscode/webview-ui-toolkit",
		);
		// Copy react build directory
		delDir("./test-webview/node_modules/@vscode/webview-ui-toolkit/react");
		createDir(
			"./test-webview/node_modules/@vscode/webview-ui-toolkit/react",
		);
		copyDir(
			"./react",
			"./test-webview/node_modules/@vscode/webview-ui-toolkit",
	...
	console.log(
		`  1. Open the ${color(["cyan"], "test-webview")} folder in a new VS Code window`,
	);
	console.log(
		`  2. Press ${color(["cyan"], "F5")} to open the webview test extension with the most recent toolkit build loaded`,
	);
	console.log(
		`  3. Run the "${color(["cyan"], "Component Gallery: Show")}" command using the VS Code command palette`,
	);
	console.log();
}
