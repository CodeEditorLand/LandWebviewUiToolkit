...
import { color, copyDir, delDir } from "./helpers.js";
...
async function main() {
	...
	try {
		...
		console.log(
			`${color(["red"], "Error: Moving the React build directory into the project root failed.")}\n    ${err}`,
		);
		process.exit();
	}
...
async function updateReactBuildImportPaths(path) {
	...
	try {
		...
		console.log(
			`${color(["red"], "Error: Reading and updating React build file import paths failed.")}\n    ${err}`,
		);
		process.exit();
	}
	...
	try {
		...
		console.log(
			`${color(["red"], "Error: Writing new React build file import paths failed.")}\n    ${err}`,
		);
		process.exit();
	}
}
